...
def convert_kernel_fft(k, seqlen):
    # replace this with a kernel
    if seqlen in [512, 2048]:
        return torch.fft.rfft(k, n=seqlen)
    else:
        return torch.fft.fft(k, n=seqlen)

class FlashFFTConvNoiFFT(torch.nn.Module):
    ...
    def forward(self, u, k_f, pregate=None, postgate=None):
        # orig_dtype = u.dtype
        ...
        if pregate is not None or postgate is not None:
            ...
            return GatedFlashFFTConvFuncNoiFFT.apply(u, k_f, self, pregate, postgate)
        return FlashFFTConvFuncNoiFFT.apply(u, k_f, self)


class FlashFFTConvFuncNoiFFT(torch.autograd.Function):

    @staticmethod
    def forward(ctx, u, k_f, fftconv_data):
...
class GatedFlashFFTConvFuncNoiFFT(torch.autograd.Function):

    @staticmethod
    def forward(ctx, u, k_f, fftconv_data, pregate, postgate):
        # assert(u.dtype == fftconv_data.dtype)
